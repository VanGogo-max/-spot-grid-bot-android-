...
def load_or_init_pnl():
    try:
        with open(PNL_FILE, "r") as f:
            data = json.load(f)
            # Увери се, че има нужните ключове
            if "initial_balance" not in data or "last_pnl_report" not in data:
                raise ValueError("Invalid PnL file")
            return data
    except Exception as e:
        logger.info("🆕 Инициализиране на P&L данни...")
        # Изчисляваме начален баланс
        total = 0.0
        for ex in EXCHANGES:
            if ex.is_active():
                try:
...
# --- ФУНКЦИЯ: Запазване на P&L данни ---
def save_pnl(data):
    with open(PNL_FILE, "w") as f:
        json.dump(data, f)

# --- ФУНКЦИЯ: Отчет за P&L ---
def report_pnl():
